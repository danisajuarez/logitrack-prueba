...
export async function GET(req: NextRequest) {
  try {
    // 1) Verificar si ayer hubo viajes en la tabla vieja
    const countQuery = `
      SELECT COUNT(*) AS cant_ayer
    ...
    if (!Number.isFinite(cantAyer) || cantAyer === 0) {
      // No hubo viajes ayer -> no traer "viajes nuevos"
      return NextResponse.json({ hasNew: false, items: [] });
    }

    // 2) Si hubo viajes ayer, seguir con la lógica actual (consulta combinada)
