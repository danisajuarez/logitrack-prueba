...
async function verificarViaje(viajeId) {
  ...
  if (viaje.length === 0) {
    ...
    console.log('\n✅ Posibles razones:');
    console.log('  • Ya fue eliminado correctamente');
  ...
  console.log('   1. Ve a la interfaz web (http://localhost:3000/viajes)');
  ...
  console.log(`      node verificar-viaje.js ${viajeId}\n`);
...
// Obtener ID del viaje de los argumentos
const viajeId = process.argv[2];

if (!viajeId || isNaN(viajeId)) {
  console.log('\n❌ Error: Debes proporcionar un ID de viaje');
  console.log('\nUso: node verificar-viaje.js <ID_VIAJE>');
  console.log('Ejemplo: node verificar-viaje.js 709\n');
  process.exit(1);
}

verificarViaje(viajeId).catch(console.error);
